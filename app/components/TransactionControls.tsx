...
export default function TransactionControls() {
  ...
  const renderChips = (current: string, setFunc: (v: string) => void) => (
    <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
      {CATEGORIES.map(c => (
        <button key={c} onClick={() => setFunc(c)} className={`whitespace-nowrap px-3 py-1 rounded-full text-xs font-medium border transition-all duration-200 active:scale-95 ${category === c ? 'bg-brand text-white border-brand shadow-sm' : 'bg-gray-50 text-gray-600 border-gray-200 hover:border-gray-300'}`}>{c}</button>
      ))}
    </div>
  );
      ...
      {isSheetOpen && (
        <div className="fixed inset-0 z-50 flex items-end justify-center">
          ...
          <div className="bg-white w-full max-w-md rounded-t-3xl p-6 z-50 animate-slide-up shadow-2xl pb-10">
            ...
            <button onClick={() => openNewModal('real')} className="w-full bg-gray-50 p-4 rounded-xl flex items-center gap-4 mb-3 active:scale-[0.98] transition-transform"><div className="w-10 h-10 rounded-full bg-green-100 text-green-600 flex items-center justify-center"><DollarSign size={20} /></div><div><h4 className="font-bold text-dark text-left">Gasto Realizado</h4><p className="text-xs text-secondary text-left">Já paguei</p></div></button>
            <button onClick={() => openNewModal('planned')} className="w-full bg-gray-50 p-4 rounded-xl flex items-center gap-4 active:scale-[0.98] transition-transform"><div className="w-10 h-10 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center"><Calendar size={20} /></div><div><h4 className="font-bold text-dark text-left">Planejar Futuro</h4><p className="text-xs text-secondary text-left">Vou pagar</p></div></button>
          </div>
        </div>
      )}
