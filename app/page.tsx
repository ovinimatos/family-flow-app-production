...
export default function Home() {
  ...
  const dashboardData = useMemo(() => {
    ...
    yearData.forEach(t => { const d = new Date(t.date + 'T12:00:00'); const m = d.getMonth(); if(t.status === 'paid') mReal[m] += Math.abs(Number(t.amount)); else mPlan[m] += Math.abs(Number(t.amount)); });
        ...
        {txLoading ? <DashboardSkeleton type="dashboard" /> : (
          <div className="px-4 pt-4 max-w-lg mx-auto animate-fade-in pb-32">
            <div className="flex items-center justify-between mb-6 bg-white p-2 rounded-xl shadow-sm border border-gray-100">
                <button onClick={() => setDashYear(d => d - 1)} className="p-2 text-gray-400 hover:text-brand rounded-full"><ChevronLeft size={20} /></button>
                <h1 className="text-lg font-extrabold text-dark">Análise <span className="text-brand">{dashYear}</span></h1>
                <button onClick={() => setDashYear(d => d + 1)} className="p-2 text-gray-400 hover:text-brand rounded-full"><ChevronRight size={20} /></button>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-6">
                <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 border-l-4 border-l-brand"><p className="text-[10px] font-bold text-secondary uppercase mb-1">Total Realizado</p><p className="text-lg font-bold text-dark">{formatCurrency(dashboardData.totalSpent)}</p></div>
            ...
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 mb-6">
                ...
                <div className="flex items-end justify-between h-48 gap-1 pb-2">
                    {dashboardData.mReal.map((realVal, idx) => {
                        ...
                        return (<div key={idx} className="w-full flex flex-col justify-end items-center h-full group"><div className={`text-[9px] font-bold ${labelColor} -mb-1 opacity-0 group-hover:opacity-100 transition-opacity`}>{label}</div><div className="w-full flex gap-[1px] items-end justify-center h-full mt-1"><div className="w-1/2 bg-brand rounded-t-sm grow-bar opacity-90" style={{height: `${realH}%`, animationDelay: `${idx*0.05}s`}}></div><div className="w-1/2 bg-gray-300 rounded-t-sm grow-bar opacity-80" style={{height: `${planH}%`, animationDelay: `${idx*0.05}s`}}></div></div></div>);
                    })}
                </div>
                <div className="flex justify-between mt-1 px-1 text-[8px] text-gray-400 font-bold uppercase">{mInitials.map((m, i) => <div key={i} className="w-full text-center">{m}</div>)}</div>
            </div>
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-24"><h3 className="text-sm font-bold text-dark mb-4">Top Categorias</h3><div className="space-y-4">{dashboardData.sortedCats.length === 0 ? (<p className="text-center text-xs text-gray-400">Sem dados.</p>) : (dashboardData.sortedCats.map(([cat, val]) => { const pct = (val / dashboardData.totalSpent) * 100; const barPct = (val / dashboardData.sortedCats[0][1]) * 100; return (<div key={cat}><div className="flex justify-between text-xs mb-1"><span class="font-bold text-dark">{cat}</span><span class="text-secondary">{formatCurrency(val)} ({pct.toFixed(0)}%)</span></div><div className="w-full bg-gray-100 rounded-full h-2"><div class="bg-brand h-2 rounded-full transition-all duration-1000 ease-out" style={{ width: `${barPct}%` }}></div></div></div>); }))}</div></div>
          </div>
        )}
      </div>
      <TransactionControls /> 
